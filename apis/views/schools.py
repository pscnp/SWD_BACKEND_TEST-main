...
from apis.serializers import SubjectsSerializer, StudentSubjectsScoreDetailsSerializer
...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if student_first_name is None or student_last_name is None or subject_title is None or score is None:
            return Response("Incomplete data", status=status.HTTP_400_BAD_REQUEST)
        if not isinstance(student_first_name, str):
            return Response("first_name must be string", status=status.HTTP_400_BAD_REQUEST)
        if not isinstance(student_last_name, str):
            return Response("last_name must be string", status=status.HTTP_400_BAD_REQUEST)
        if not isinstance(subject_title, str):
            return Response("subject_title must be string", status=status.HTTP_400_BAD_REQUEST)
        if not (isinstance(score, int) or isinstance(score, float)):
            return Response("score must be number", status=status.HTTP_400_BAD_REQUEST)
        if not (0 <= score <= 100):
            return Response("score must be in between 0 and 100", status=status.HTTP_400_BAD_REQUEST)
        score = float(score)

        if not Personnel.objects.filter(first_name=student_first_name, last_name=student_last_name).exists():
            return Response("Student not found", status=status.HTTP_400_BAD_REQUEST)
        if not Subjects.objects.filter(title=subject_title).exists():
            return Response("Subject not found", status=status.HTTP_400_BAD_REQUEST)

        student_id = Personnel.objects.get(first_name=student_first_name, last_name=student_last_name).id
        subjects_id = Subjects.objects.get(title=subject_title).id

        credit = None
        if subjects_id in (1, 5):
            credit = 3
        elif subjects_id in (2, 4):
            credit = 2
        elif subjects_id == 3:
            credit = 1

        if StudentSubjectsScore.objects.filter(student_id=student_id, subjects_id=subjects_id).exists():
            obj = StudentSubjectsScore.objects.get(student_id=student_id, subjects_id=subjects_id)
            obj.score = score
            obj.save()
        else:
            added_data = StudentSubjectsScore.objects.create(
                credit=credit, score=score, student_id=student_id, subjects_id=subjects_id)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        def calculate_average_grade(credits, grades):
            grade_values = {
                'A': 4,
                'B+': 3.5,
                'B': 3,
            ...
            grade_weight = [credit*value for credit, value in zip(credits, values)]
            avg_grade = sum(grade_weight)/sum(credits)
            return avg_grade

        student_id = kwargs.get("id", None)
        queryset = StudentSubjectsScore.objects.filter(student_id=student_id)
        ...
        for subject in queryset:
            subject_data.append({
                ...
                "score": subject.score,
                "grade": get_grade(subject.score),
            })

        credits = [subject.credit for subject in StudentSubjectsScore.objects.filter(student_id=student_id)]
        grades = [get_grade(subject.score) for subject in StudentSubjectsScore.objects.filter(student_id=student_id)]

        output = {
            "student": student_data,
            "subject_detail": subject_data,
            "grade_point_average": calculate_average_grade(credits, grades)
        }
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = Personnel.objects.filter(school_class__school__title=school_title).all().order_by(
            'personnel_type', 'school_class__class_order', 'first_name', 'last_name')
        output = []
        role = ['Teacher', 'Head of the room', 'Student']
        for i, person in enumerate(queryset):
            output.append(
                f"{i+1}. school: {school_title.capitalize()}, role: {role[person.personnel_type]}, class: {person.school_class.class_order}, name: {person.first_name.capitalize()} {person.last_name.capitalize()}")
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school in Schools.objects.all():
            ...
            for class_ in Classes.objects.filter(school_id=school.id):
                teacher = Personnel.objects.get(personnel_type=0, school_class_id=class_.id)
                school_dict['class '+str(class_.class_order)] = {
                    f"Teacher: {teacher.first_name} {teacher.last_name}": [
                        {role[student.personnel_type]: f"{student.first_name} {student.last_name}"} for student in Personnel.objects.filter(school_class_id=class_.id).exclude(personnel_type=0)
                    ]
                }
            output.append(school_dict)
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        output = []
        for school_division in SchoolStructure.objects.filter(parent_id__isnull=True):
            division_dict = dict()
            division_dict['title'] = school_division.title
            sub = []
            for class_level in SchoolStructure.objects.filter(parent_id=school_division.id):
                sub.append(
                    {
                        "title": class_level.title,
                        "sub": [{"title": classroom.title} for classroom in SchoolStructure.objects.filter(parent_id=class_level.id)]
                    }
                )
            division_dict['sub'] = sub
            output.append(division_dict)
